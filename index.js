...
app.get('/search', async (req, res) => {
    try {
        ...
        resend.emails.send({
            ...
            html: `<p>Productos encontrados para ${q}:</p><ul>${productos.map(producto => `<li>${producto.modelo} - ${producto.precioRebajado} - ${producto.rebaja} - ${producto.precioOriginal}</li>`).join('')}</ul>`
            });
        res.json({
